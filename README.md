...
├── identity/
│   ├── models.py          # IdentityMode, IdentityRequest, eleven observation models
│   ├── capabilities.py    # Eleven typed identity capability definitions
│   ├── transport.py       # Deterministic mock identity directory (no queries, no mutation)
│   ├── normalization.py   # Tool output → normalized observations
│   ├── evidence.py        # Observation → evidence (mode-aware dedup)
│   ├── materializer.py    # Observation → world model facts (no attack-graph edges)
│   ├── redaction.py       # Bound redaction (literal REDACTED marker)
│   └── engine.py          # IdentityEngine (capability orchestration + auth + identity resolution)
├── cloud/
│   ├── models.py          # CloudMode, CloudRequest, observation models
│   ├── capabilities.py    # Twenty typed cloud capability definitions
│   ├── transport.py       # Deterministic mock cloud transport (no network I/O)
│   ├── normalization.py   # Tool output → normalized observations
│   ├── evidence.py        # Observation → evidence (mode-aware dedup)
│   ├── materializer.py    # Observation → world model facts (no attack-graph edges)
│   ├── redaction.py       # Bound redaction (literal REDACTED marker)
│   ├── addressing.py      # Provider/account/project/asset target parsing
│   ├── providers.py       # Provider registry + identity resolution
│   └── engine.py          # CloudEngine (capability orchestration + auth)
├── container/
│   ├── models.py          # ContainerMode, ContainerRequest, observation models
│   ├── capabilities.py    # Fourteen typed container capability definitions
│   ├── transport.py       # Deterministic mock cluster transport (no cluster access)
│   ├── normalization.py   # Tool output → normalized observations
│   ├── evidence.py        # Observation → evidence (mode-aware dedup)
│   ├── materializer.py    # Observation → world model facts (no attack-graph edges)
│   ├── redaction.py       # Bound redaction (literal REDACTED marker)
...
tests/                    # Test suite (current: 1348 passed, 10 skipped, incl. all phase tests)
```
...
| Full test suite | **1348 passed, 10 skipped, 0 failed** (`python -m pytest tests/ -q`), covering all phases including the 29 orchestration + 27 development-console + 19 Layer A real-validation phase-14.1 tests; the 10 skips are environment-gated: 5 opt-in Layer B real-integration (`BLACKFORGE_REAL_TARGET` / `BLACKFORGE_REAL_MODEL`), 1 real-model smoke (`--run-integration`), and 4 backend-restart-only marks (sqlite-pair / in-memory) |
...
| Phase notebooks | Phase 1–14.1 notebooks executed; the Phase 14.1 notebook was rewritten for the Phase 14.1 remediation (execution modes, findings, HTTP console + tunnel, real Qwen); its JSON/syntax is validated and its deterministic cells are covered by the Layer A test suite locally — the network-dependent cells (real DNS, public tunnel, Qwen inference) run in the notebook on a Colab runtime with internet |
