...
class EvidenceSource(str, Enum):
    """Provenance of an evidence row: did a real transport produce it?

    ``MOCK`` marks evidence generated by a deterministic mock engine; ``REAL``
    marks evidence produced by a real-controlled adapter bound to a live
    transport. This module owns the enum so ``evidence`` stays a leaf package
    and no cycle forms with ``orchestration``.
    """
...
class Provenance(BaseModel):
    ...
    mode: EvidenceSource = EvidenceSource.MOCK
